...
Then(/^executing the Person scenario$/) do
  puts "in step definition block"
  compare = Compare_response.new 
  compare.person 
end
 ...
 def validate_200
  ...
  def run_AssetFunction
    ...
    $grades={"title"=>"00000057_SD - Test Asset for API_57_Automation","asset_type"=>"asset",
      ...
      "programme_ids"=>"00000057", "id"=>"ZscGZ5dToGYaNIWpluAbQwZnFDOJtHsy","test"=>"123"}
    ...
    CSV.foreach("./features/Asset_csv_file.csv", :headers => true) do |col|
      @hash_key= col[1]
      @path = col[2]
      @validation=col[3]
      $element = JsonPath.new(@path).on(JSON.parse(response.body))
...
def offer
 ...
 CSV.foreach("./features/Offer_csv_file.csv", :headers => true) do |col|
  @hash_key= col[1]
  @path = col[2]
  @validation=col[3]
  $element = JsonPath.new(@path).on(JSON.parse(response.body))
...
def programme
 ...
 $grades={"imdbid"=>"0369610","programme_title"=> "TV_Eps_Test Asset for API_51_Automation", "type"=>"programme",
  ...
  @request_url=$base_url + relative_path
  response = Net::HTTP.get_response(URI.parse(@request_url))
  CSV.foreach("./features/Programme_csv_file.csv", :headers => true) do |col|
   @hash_key= col[1]
   @path = col[2]
   @validation=col[3]
   $element = JsonPath.new(@path).on(JSON.parse(response.body))
...
def string
  ...
  $grades={"title"=>"String_API_Automation","machine_name"=>"machine_18","string"=>"Watch Now",
    ...
    @request_url=$base_url + relative_path
    response = Net::HTTP.get_response(URI.parse(@request_url))
    CSV.foreach("./features/String_csv_file.csv", :headers => true) do |col|
      @hash_key= col[1]
      @path = col[2]
      @validation=col[3]
      $element = JsonPath.new(@path).on(JSON.parse(response.body))
...
def review
  ...
  @request_url=$base_url + relative_path
  response = Net::HTTP.get_response(URI.parse(@request_url))
  CSV.foreach("./features/Review_csv_file.csv", :headers => true) do |col|
    @hash_key= col[1]
    @path = col[2]
    @validation=col[3]
    $element = JsonPath.new(@path).on(JSON.parse(response.body))
...
def season
  ...
  @request_url=$base_url + relative_path
  response = Net::HTTP.get_response(URI.parse(@request_url))
  CSV.foreach("./features/Season_csv_file.csv", :headers => true) do |col|
    @hash_key= col[1]
    @path = col[2]
    @validation=col[3]
    $element = JsonPath.new(@path).on(JSON.parse(response.body))
...
@request_url=$base_url + relative_path
response = Net::HTTP.get_response(URI.parse(@request_url))
CSV.foreach(".features/series_csvfile.csv", :headers => true) do |col|
  @path = col[2]
  @hash_key= col[1]
  @validation=col[3]
  $element = JsonPath.new(@path).on(JSON.parse(response.body))
...
def genre_function
  ...
  $grades = {:programeIds=>["1442992948", "1442992950"], "1442992948"=>{:start_date=>"2006-01-01T00:00:00.000Z", 
    ...
    :links_ooyala_genreProgrammes_templated =>true, :links_ooyala_genreSeries_templated=>true,
    ...
    :embedded_ooyala_programme_0_image_landscape_mask=>"0.00", :embedded_ooyala_programme_0_ptype=>"tv",
    :embedded_ooyala_programme_0_sea_pub=>"published", :embedded_ooyala_programme_0_series_id=>"51234",
    ...
    :embedded_ooyala_programme_0_embedded_ooyala_offer_0_device_ids_0=>"0f51bce0bd3bb505c5d7ae27c755354a33aafbec",
...
puts "relative url is #{relative_path}"
@request_url=$base_url + relative_path
response = Net::HTTP.get_response(URI.parse(@request_url))
CSV.foreach("E:/EndToEndAPiDoc/E2E_Automation code/Cucumber/features/Genre_csv_file.csv", :headers => true) do |col|
  @path = col[2]
  @hash_key= col[1]
  @validation=col[3]
  $element = JsonPath.new(@path).on(JSON.parse(response.body))
...
def person
  puts "For ooyala:person API call"
  $grades = {:programeIds=>["1442992948", "1442992950"], "1442992948"=>{:start_date=>"2006-01-01T00:00:00.000Z", 
    :end_date=>"2021-01-01T00:00:00.000Z", :offerType=>"PPV-Promotion-699", 
    :deviceId=>["STBmanaged", "STBunmanaged", "CEMediaPlayer", "gameConsole", "tablet", "phone"],  
    :series_id=>"61234",:title => "Test Asset for API_51_Automation1",  :providerId=>"Studio", 
    :Title=>"E2EAutomationAssetMovie", 
    :VideoFormat=>["SD", "HD"], :person_title=>"Steele Stebbins", :programme_ids=>"30000122", 
    :programme_roles=>"30000122", :person_id=>"Steele Stebbins", 
    :links_ooyala_personProgrammes_templated=>true
  }
}
partial_relative_path="#{$grades[$grades[:programeIds][0]][:person_id]}"
...
puts "relative_path is #{relative_path}"
    #@request_url= $base_url+relative_path
    @request_url="http://foxtel-api.local/person/Steele%20Stebbins"
    puts "value of request_url is #{@request_url}"
    response = Net::HTTP.get_response(URI.parse(@request_url))
    CSV.foreach("./features/person_csv_file.csv", :headers => true) do |col|
      @path = col[2]
      @hash_key= col[1]
      @validation=col[3]
      $element = JsonPath.new(@path).on(JSON.parse(response.body))
